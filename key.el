...
(use-package devil
  ...
  (devil-repeatable-keys '(("%k p" "%k n" "%k b" "%k f" "%k a" "%k e")
			   ;; repeatable keys for window and buffer management
			   ;; and find-file and dired 
			   ;; I went a little crazy here but since most of the
			   ;; time actions on buffers 
			   ;; and windows are followed by other commands like
			   ;; "C-n" I believe it's ok 
			   ("%k c k" "%k x o" "%k x &" "%k x é" "%k x \"" "%k x
à" "%k x %k f" "%k x d" "%k x b")
