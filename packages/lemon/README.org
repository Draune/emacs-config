* Lemon
  ...
  the echo area when Emacs is has been idle for a few seconds.

  This is a fork of [[https://github.com/zk-phi/symon][zk_phi’s Symon]], which has been largely rewritten.
...
*** Global options
    ...
    - =lemon-monitors= :: List of list of monitors.

    Each outer list is a page.  Lemon rotates through pages as it refreshes
    (every =lemon-refresh-rate= seconds).
    ...
    - =lemon-delay= :: Delay in seconds until Lemon appears.

    - =lemon-refresh-rate= :: How often to obtain new values from the
         monitors.
...
** Standard monitors
   ...
   - [[file:lemon-battery.el][=lemon-battery=]] :: Displays battery and charging status.
   - [[file:lemon-cpu.el][=lemon-cpu=]] :: Monitors CPU load.
   - [[file:lemon-cpufreq.el][=lemon-cpufreq=]] :: Monitors CPU frequency.
   - [[file:lemon-fan.el][=lemon-fan=]] :: Monitors fan speed.
   - [[file:lemon-memory.el][=lemon-memory=]] :: Displays memory usage.
   - [[file:lemon-swap.el][=lemon-swap=]] :: Displays swap usage.
   - [[file:lemon-temp.el][=lemon-temp=]] :: Monitors temperatures.
   - [[file:lemon-time.el][=lemon-time=]] :: Display a clock in Lemon.
...
** Creating new monitors
   ...
   Lemon uses EIEIO classes to implement monitors, and provides two
   base classes to build on:

   - =lemon-monitor= :: This is the base class, suitable for monitors
        that display a single value.

   - =lemon-monitor-history= :: This class keeps a history of values,
...
*** Defining the class
    ...
    #+BEGIN_SRC emacs-lisp :eval never
      (defclass lemon-hello (lemon-monitor))
    #+END_SRC
...
*** Fetching values
    ...
    The =lemon-monitor-fetch= generic function evaluates to the
    ...
    #+BEGIN_SRC emacs-lisp :eval never
      (cl-defmethod lemon-monitor-fetch ((this lemon-hello))
        user-login-name)
    #+END_SRC
...
*** Displaying the value
    ...
    The =lemon-monitor-display= generic function formats the value
    ...
    #+BEGIN_SRC emacs-lisp :eval never
      (cl-defmethod lemon-monitor-display ((this lemon-custom-monitor))
        (with-slots (greeting) this
          (propertize
           (format "%s, %s" greeting (lemon-monitor-value this))
           'face 'compilation-info)))
    #+END_SRC

   The default implementation of =lemon-monitor-display= is generic
...
*** Constructing the monitor
    ...
    #+BEGIN_SRC emacs-lisp :eval never
      (setq my/hello-monitor (lemon-hello :greeting "Hi"))
    #+END_SRC
...
** Contributors
   ...
   - zk_phi is the original author of Lemon.
   - [[https://github.com/pierre-lecocq][Pierre Lecocq]] added Darwin support.
   - Ian Eure substantially rewrote and extended Lemon.
